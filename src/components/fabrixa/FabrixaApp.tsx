...
import {
  ...
  User as UserIcon, Crown, Calendar, Palette, SlidersHorizontal, Trash2,
} from "lucide-react";
...
import {
  ...
  type GarmentTypeId, type PartState, type DesignLayer
...
import {
  newLayerId, recompositeLayerOverlay, recompositeAllAffected, partKeysWithLinkGroup,
...
export function FabrixaApp() {
  ...
  const [lassoMode, setLassoMode] = useState<"freehand" | "polygon" | "brush">("freehand");
  const [lassoBrushSize, setLassoBrushSize] = useState(24);
  ...
  useEffect(() => {
    if (Object.keys(partStates).length > 0) {
    ...
    await runGated(feature, async () => {
      ...
      if (activeLayerId) {
        ...
        if (idx !== -1 && !nextLayers[idx].locked) {
          const updated = {
            ...nextLayers[idx],
            contentDataUrl: designUrl,
            contentJson: designJson || undefined,
          };
          nextLayers[idx] = updated;
          // Sync linked layers across parts
          ...
          if (gid) {
            ...
            for (const key of affected) {
              ...
              ls[li] = { ...ls[li], contentDataUrl: designUrl, contentJson: designJson || undefined };
  ...
  const handleLassoMask = (key: string, dataUrl: string, triCount: number) => {
    ...
    toast.success(`Selected ${triCount} surface(s) on ${activeLabel}. Create a layer or edit in 2D.`);
  };
  ...
  const applyToSelection = async () => {
    ...
    await runGated("MASKED_APPLY", async () => {
      ...
      const id = activeLayerId && nextLayers.find((l) => l.id === activeLayerId && !l.locked)
        ? activeLayerId
        : newLayerId();
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      composeSelectionFillPreview().then((url) => { if (!cancelled) setRegionPreviewUrl(url); }).catch(() => {});
    }, 120);
    return () => { cancelled = true; clearTimeout(t); };
    ...
    <TooltipProvider delayDuration={300}>
      <div className="flex h-screen flex-col bg-background text-foreground">
        ...
        <header className="flex h-14 shrink-0 items-center gap-1.5 border-b border-white/10 bg-background/60 px-2 backdrop-blur-2xl shadow-sm sm:gap-2 sm:px-4 z-50">
          ...
          <Tabs value={view} onValueChange={(v) => void changeView(v as "design" | "preview" | "ai")} className="sm:ml-4">
          ...
          <div className="ml-auto flex min-w-0 items-center gap-1">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56 bg-background/80 backdrop-blur-2xl border-white/10">
                ...
                <DropdownMenuItem onClick={() => quickExport("tiled")}><Grid3x3 className="mr-2 h-4 w-4" />Tiled Repeat</DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuLabel className="text-[10px] uppercase tracking-wider text-muted-foreground">Realistic Renders (3D)</DropdownMenuLabel>
                <DropdownMenuItem onClick={() => handleRealisticExport("front")}>
                  <Shirt className="mr-2 h-4 w-4" /> Export Front View
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleRealisticExport("back")}>
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          >
            <FabricEditor
              ...
              onChange={(url, json) => { setDesignUrl(url); setDesignJson(json); }}
              initialMaskUrl={activeLayerId ? partStates[activePart]?.layers?.find(l => l.id === activeLayerId)?.maskUrl : null}
              initialJson={activeLayerId ? partStates[activePart]?.layers?.find(l => l.id === activeLayerId)?.contentJson : null}
              initialContentUrl={activeLayerId ? partStates[activePart]?.layers?.find(l => l.id === activeLayerId)?.contentDataUrl : null}
            ...
            <div className="grid h-full grid-cols-1 gap-3 overflow-auto p-3 lg:grid-cols-[1fr_320px]">
                ...
                <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-white/10 bg-background/40 p-2 backdrop-blur-xl shadow-lg">
                  ...
                  <div className="ml-auto flex flex-wrap items-center gap-3">
                    <Button
                      ...
                      onClick={() => { setLassoMode("brush"); setLassoActive((v) => lassoMode === "brush" ? !v : true); }}
                    ...
                    {lassoActive && lassoMode === "brush" && (
                      <div className="flex items-center gap-1.5 w-28">
                        ...
                        />
                        <span className="text-[9px] tabular-nums text-muted-foreground">{lassoBrushSize}</span>
                      </div>
                    )}
                    <Button
                      size="sm"
                      variant={lassoActive && lassoMode === "freehand" ? "default" : "outline"}
                      onClick={() => { setLassoMode("freehand"); setLassoActive((v) => lassoMode === "freehand" ? !v : true); }}
                      className="h-7 gap-1 text-xs bg-background/50 border-white/10"
                    >
                    ...
                    <Button
                      ...
                      onClick={() => { setLassoMode("polygon"); setLassoActive((v) => lassoMode === "polygon" ? !v : true); }}
                      className="h-7 gap-1 text-xs bg-background/50 border-white/10"
                    >
                      <Scissors className="h-3.5 w-3.5" />
                    ...
                    {activeState.selectionMaskDataUrl && (
                      <div className="flex items-center gap-1 animate-in fade-in zoom-in ml-2">
                        <Button size="sm" variant="default" onClick={handleEditSelection} className="h-7 text-xs bg-primary text-primary-foreground shadow-lg shadow-primary/20">
                          <Palette className="h-3.5 w-3.5 mr-1" />
                          Edit in 2D
                        </Button>
                        <Button size="sm" variant="ghost" onClick={clearLassoMask} className="h-7 text-xs text-destructive">
                          Clear
                        </Button>
                      </div>
                    )}
                    <Sep />
                    <div className="flex items-center gap-1">
                      <Button size="sm" variant="outline" onClick={() => handleRealisticExport("front")} className="h-7 px-2 text-[10px] bg-background/50 border-white/10">
                        Front Render
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => handleRealisticExport("back")} className="h-7 px-2 text-[10px] bg-background/50 border-white/10">
                        Back Render
                      </Button>
                    </div>
                    <Sep />
              ...
              <div className="rounded-2xl border border-white/10 bg-background/50 p-4 backdrop-blur-2xl shadow-2xl">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <Button size="sm" variant="outline" className="bg-background/40 border-white/10" onClick={() => void changeView("design")}>
                ...
                <div className="mb-3 aspect-square w-full overflow-hidden rounded-md border border-white/10 bg-[conic-gradient(at_50%_50%,#e9e9ef_25%,#fafafa_0_50%,#e9e9ef_0_75%,#fafafa_0)] bg-[length:16px_16px]">
                  {activeState.layerCompositeDataUrl || activeState.textureDataUrl
                    ? <img src={activeState.layerCompositeDataUrl || activeState.textureDataUrl || ""} alt="texture" className="h-full w-full object-cover" />
                ...
                <div className="space-y-3">
                  ...
                  <SliderRow label="Pattern X offset" value={activeState.textureOffsetX ?? 0} min={-1} max={1} step={0.01} onChange={(v) => updateActivePart({ textureOffsetX: v })} />
                  <SliderRow label="Pattern Y offset" value={activeState.textureOffsetY ?? 0} min={-1} max={1} step={0.01} onChange={(v) => updateActivePart({ textureOffsetY: v })} />
                  <SliderRow
...
function Sep() { return <div className="hidden sm:block mx-1 h-6 w-px shrink-0 bg-white/10" />; }
