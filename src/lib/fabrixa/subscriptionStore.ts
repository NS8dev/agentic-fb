    ...
    (set, get) => ({
      ...
      setAdminMode: (on) => {
        if (on) {
          set({
            adminMode: true,
            // Grant a starting balance if they have none, so deduction can be tested
            coinBalance: Math.max(get().coinBalance, 5000),
            dailyAiRequestsRemaining: Math.max(get().dailyAiRequestsRemaining, 100),
          });
        } else {
          set({ adminMode: false });
        }
      ...
      spend: (action, qty = 1) => {
        ...
        if (action === "aiGeneration") {
          ...
          const tier = s.subscriptionTier ? APP_DATA_0.tiers[s.subscriptionTier] : null;
          if (!s.adminMode && tier && !tier.aiIncluded && !s.hasAiPack) {
      ...
      registerShowroomDownload: (designId) => {
        const s = get();
        if (s.unlockedShowroomDesigns.includes(designId)) return { ok: true };
        if (s.isExpired()) return { ok: false, reason: "Subscription expired" };
        ...
        if (!s.adminMode) {
          ...
          if (s.dailyShowroomDownloadsCount >= tier.maxShowroomDownloadsPerDay) {
            return {
              ok: false,
              reason: `Daily showroom download limit reached (${tier.maxShowroomDownloadsPerDay})`,
            };
          }
        }
        ...
        set({
          ...
          unlockedShowroomDesigns: [...s.unlockedShowroomDesigns, designId],
        });
        return { ok: true };
      },
