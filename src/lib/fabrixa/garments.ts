...
export interface DesignLayer {
  ...
  locked?: boolean;
  /** Shared id across parts — linked layers sync design content. */
...
export interface PartState {
   ...
   *  tile). Used when tileGap > 0 for the base fabric map only. */
  texturePaddedDataUrl?: string | null;
  /** Composited transparent design layers (UV-mapped overlay). Independent from
   *  the base fabric texture and tile-gap padding. */
  ...
  /** "uv" = world-space continuous fabric (triplanar, textureScale slider).
   *  "world" = triplanar world-space sampling (worldTilingScale slider).
   *  Both modes tile seamlessly across all mesh parts; design layers stay UV-mapped. */
  tilingMode?: "uv" | "world";
  /** World units per tile (only used in tilingMode="world"). Smaller = denser. */
  worldTilingScale?: number;
  /** Environment map reflection intensity (0..2). Higher = more shiny/reflective. */
  reflectionIntensity?: number;
  /** Surface roughness (0..1). 0 = smooth/glossy, 1 = matte/rough. */
  roughness?: number;
