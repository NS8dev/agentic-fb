...
class TextureCache {
  ...
  acquire(src: string): THREE.Texture {
    ...
    tex.flipY = false; // CRITICAL: match glTF V=0 at top UV layout.
  ...
  release(src: string, tex?: THREE.Texture | null) {
    ...
    if (tex && entry.textures.has(tex)) {
      ...
      // Fallback for when we don't have the tex pointer but know we want to release a slot
      entry.refCount -= 1;
    }
...
/** Convert UV-mode repeat count to world metres-per-tile for triplanar fabric. */
export function textureScaleToWorldScale(textureScale: number): number {
  const garmentRefHeight = 1.6;
  return garmentRefHeight / Math.max(0.5, textureScale);
}

/**
 * Acquire a texture for UV-mapped design layer overlays.
 * Uses ClampToEdge so decals don't tile/repeat outside 0–1 UV space.
 */
export function acquireLayerTexture(src: string): THREE.Texture {
  const tex = textureCache.acquire(src);
